...
from policyproof.evidence_sufficiency_case_construction import (
    ...
    write_case_construction_json_artifact,
)
...
def test_writer_publishes_formatted_utf8_json(
    ...
    output_path = tmp_path / "nested" / "case-construction.json"
...
def test_writer_refuses_to_overwrite_existing_output(
    ...
    output_path = tmp_path / "case-construction.json"
...
def test_writer_rejects_non_mapping_artifact(
    ...
    with pytest.raises(
        ...
        write_case_construction_json_artifact(
            ...
            tmp_path / "case-construction.json",
        )
...
def test_writer_rejects_non_path_output() -> None:
    with pytest.raises(
        ...
        write_case_construction_json_artifact(
            sample_artifact(),
            "case-construction.json",  # type: ignore[arg-type]
        )
...
def test_writer_removes_temporary_file_after_publish_failure(
    ...
    def fail_link(
        ...
        raise OSError(f"simulated publication failure: {source} -> {destination}")
    ...
    assert list(tmp_path.glob(".case-construction.json.*.tmp")) == []
