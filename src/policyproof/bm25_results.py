"""Versioned result artifacts for the deterministic BM25 baseline."""
...
from policyproof.bm25_evaluation import BM25EvaluationResult
...
RESULT_ID = "policyproof-bm25-baseline"
VERSION_PATTERN = re.compile(r"[0-9]+\.[0-9]+\.[0-9]+")
SHA256_PATTERN = re.compile(r"[0-9a-f]{64}")


class BM25ResultError(ValueError):
    """Raised when a BM25 result artifact cannot be built or written."""
...
def _require_mapping(
    ...
    *,
    ...
    if not isinstance(value, Mapping):
        raise BM25ResultError(f"{field_name} must be a mapping.")
...
def build_bm25_result_artifact(
    evaluation: BM25EvaluationResult,
    *,
    ...
    """Serialize a validated BM25 evaluation with immutable input bindings."""

    if not isinstance(evaluation, BM25EvaluationResult):
        raise BM25ResultError(
            "evaluation must be a BM25EvaluationResult."
        )
    ...
    for result in evaluation.query_results:
        ...
        for rank, hit in enumerate(
            ...
            if not math.isfinite(hit.score) or hit.score < 0:
                raise BM25ResultError(
                    f"{result.query_id}: ranked score must be "
...
def write_bm25_result_artifact(
    artifact: Mapping[str, Any],
    path: Any,
) -> None:
    """Write one BM25 result artifact atomically without overwriting."""
    ...
    try:
        ...
        raise BM25ResultError(str(error)) from error
